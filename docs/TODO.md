...
### 2026-06-04（阵容分析模块 + 群聊）
- **阵容分析模块**：拉取《侠客2》玩家离线大码 → protobuf 解析 → 按门派/段位统计物品选用率。表 `lineup_snapshots` / `lineup_fetch_jobs` / `lineup_schedule_config`
- **使用率口径**：次数 → 新增百分比（次数/总回合数×100，可>100%）；顶部统计拆「总局数 + 总回合数」（新增 `round_count` 字段并回填）
- **可视化**：物品使用率横向柱状图、职业物品占比饼图（独立拉取，不依赖"仅职业"勾选）、按物品类型分组 Top20/Last20（空类型隐藏）、各门派职业物品占比 5×2 饼图网格（前3标名）
- **物品名按品质着色**：解析 ItemCfg 的 Rank 字段（tag 9），普通绿/精良蓝/上乘紫/传说橙/绝世红；职业物品柱改回蓝色 + 名字左侧 ★
- **系列物品合并**：同物品不同等级折叠为系列（野球拳/修罗印/巡捕令系列），显式 ID 映射表 `lineup/series.py`
- **选用失衡度分析**：基尼系数（≥0.6 严重）+ 相对公平份额法（份额<公平份额1/N 的 25%=偏冷、10%=严重偏冷）；按类型失衡 + 职业物品前3失衡（每门派只算选用最高前3，固定对比10门派）
- **失衡排除名单**：BOSS 掉落稀缺物品不计入失衡统计，维护表 `lineup/exclusions.py`；仅影响失衡，不影响使用率图表
- **聚合查询缓存**：usage/usage-by-type/stats/失衡端点加内存缓存，指纹=（行数, max(last_seen_at)），数据未变切页签毫秒级
- **拉取频率/清理**：默认每 1 小时一轮，每轮后清理 `last_seen_at` 超 7 天的快照
- **幽灵任务清理**：启动时把残留 `running` 的 `LineupFetchJob` 标 failed（仿 CrawlJob）
- **群聊**：同游戏登录用户实时聊天室，表 `chat_messages`，HTTP 短轮询（3s）+ 乐观更新
- **QQ / 小黑盒时间晚 8 小时修复**：`fromtimestamp` → `utcfromtimestamp`（项目约定库内存 UTC），存量数据 SQL 回修 -8 小时
