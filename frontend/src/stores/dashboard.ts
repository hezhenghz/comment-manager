...
export const useDashboardStore = defineStore('dashboard', {
  state: () => ({
    ...
    categories: [] as any[],
    sources: [] as any[],
    aiQueue: { pending: 0, failed: 0, skipped: 0, done: 0 } as AiQueueStats,
  }),
  actions: {
    async loadAll(gameId?: string | null) {
      ...
      const [stats, categories, sources, aiQueue] = await Promise.allSettled([
        api.get('/dashboard/stats', { params }),
        api.get('/dashboard/categories', { params }),
        api.get('/dashboard/sources', { params }),
        api.get('/dashboard/ai-queue', { params }),
      ]);
      ...
      if (aiQueue.status    === 'fulfilled') this.aiQueue    = aiQueue.value.data;
    },
  },
});
